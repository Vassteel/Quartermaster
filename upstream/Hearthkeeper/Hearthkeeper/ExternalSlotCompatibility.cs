...
internal static class ExternalSlotCompatibility
{
	...
	internal static bool IsProtectedPlayerSlot(int x, int y)
	{
		...
		{
			HearthkeeperPlugin.Log?.LogWarning("Equipment and Quick Slots compatibility check failed: " + ex.GetBaseException().Message);
	...
	private static void EnsureEaqsApi()
	{
		if (!_lookupAttempted)
		{
			...
			if (_eaqsIsSlotCell != null && !_availabilityLogged)
			{
				...
				HearthkeeperPlugin.Log?.LogInfo("Equipment and Quick Slots compatibility enabled; equipment, quick, and custom slots are protected.");
			}
		}
	}
}
