...
var worldMaterial=new Material(piece);worldMaterial.SetTexture("_MainTex",new Texture());
...
var feathers=GullMaterials.Feathers(original,worldMaterial);
Check(feathers.shader==piece,"uses the loaded chest shader reference");
...
Check(!feathers.keywords.Contains("UNRELATED_CREATURE_VARIANT"),"does not inherit creature variants into the chest lighting shader");
...
Check(GullMaterials.Feathers(original,worldMaterial).shader==piece,"missing Shader.Find entries cannot hide the native gull");
...
NonEmissive(GullMaterials.Feathers(original,worldMaterial));
